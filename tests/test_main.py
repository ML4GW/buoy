...
class TestResolveEvents:
    ...
    def test_warns_when_events_and_runs_both_provided(
        self, MockAframe, MockAmplfi, mock_get_data, tmp_path, caplog
    ):
        model = _make_mock_model()
        MockAframe.return_value = model
        MockAmplfi.return_value = model
...
class TestMainLoop:
    ...
    def test_skips_event_when_outputs_exist(
        self, mock_get_data, mock_models, skipped_event_dir
    ):
    ...
    @patch("buoy.main.get_data")
    def test_skips_event_when_run_amplfi_false(
        self, mock_get_data, mock_models, tmp_path
    ):
